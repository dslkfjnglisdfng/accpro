...
class ParametricModel:
    ...
    def get_bone_vector(self):
        """
        获取所有骨骼的朝向和长度（基于 T-pose）

        Returns
        -------
        bone_len : torch.Tensor, shape [num_joint]
            每根骨骼的长度
        bone_vec : torch.Tensor, shape [num_joint, 3]
            原始骨向量（未归一化）
        """
        # T-pose 下的关节位置（root 已在原点）
        j, _ = self.get_zero_pose_joint_and_vertex()  # j: [num_joint, 3]
        j = j.unsqueeze(0)
        # bone_vec[i] = p_i - p_parent(i)
        bone_vec = self.joint_position_to_bone_vector(j)  # [num_joint, 3]
        bone_vec = bone_vec.squeeze(0)
        # 长度
        bone_len = torch.norm(bone_vec, dim=-1)  # [num_joint]


        return  bone_len, bone_vec
    ...
    def get_se3_from_so3pose(self,pose: torch.Tensor, shape: torch.Tensor = None):
        """
        我们在这里从so3空间下的pose[rotation]参数来获得se3的[rotaition,position]矩阵
        param pose: Joint local rotation tensor in shape [batch_size, *] that can reshape to
                     [batch_size, num_joint, 3, 3] (rotation matrices)
        param shape: Tensor for model shapes that can expand to [batch_size, 10]. Use None for the mean(zero) shape.
        return:
        T_local_se3_motion in [batch_size,num_joint,4,4]
        """
        pose = pose.view(pose.shape[0], -1, 3, 3)
        j, v =  [_.expand(pose.shape[0], -1, -1) for _ in self.get_zero_pose_joint_and_vertex(shape)]
        T_local = M.transformation_matrix(pose, self.joint_position_to_bone_vector(j))
